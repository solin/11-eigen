...
\title{Adaptive $hp$-FEM for Eigenvalue Problems\\ that Avoids Repeated 
       Eigensolver Calls}
...
an eigensolver in each adaptivity step. This is not practical if one is only interested 
in resolving accurately one eigenpair, such as one specific vibration mode. 
We propose a novel adaptive algorithm 
that only calls the eigensolver once at the beginning of the computation, and then employs 
an iterative method to pursue a selected eigenvalue-eigenvector pair on a sequence 
of locally refined meshes. The Picard's and Newton's variants of the iterative 
method are presented. The method works reliably even if the eigenvalue-eigenvector pair 
of interest is part of a multiple eigenvalue. The underlying partial differential 
equation (PDE) is discretized with higher-order finite elements ($hp$-FEM) but 
the algorithm also works for standard low-order FEM. The method is described and 
accompanied with theoretical analysis and numerical examples. Instructions on how to 
reproduce the results are provided. 
\end{abstract}

\begin{keywords} 
Partial differential equation, Eigenvalue problem, Adaptive higher-order finite element
...
\thispagestyle{plain}
\markboth{S.~GIANI AND P.~SOLIN}{Adaptive $hp$-FEM for Eigenvalue Problems}
...
additional eigenvalues and possibly eigenvectors need to be computed. These unwanted eigenpairs are 
...
(FEM), and their application on sequences of locally refined meshes leads to specific problems. 
...
\subsection{Case for adaptivity}\label{ssec:n_adapt}

Now, we would like to illustrate that in general each eigenfunction should be approximated separately, because the adaptation of the mesh could vary considerably. This is because the adaptive procedure is able to capture the specific features of the eigenfunction and then adapt the mesh accordingly.

\begin{figure}[!ht]
\begin{center}
\includegraphics[width=0.45\textwidth]{img/l-shape_eig1.png}\ \ \ 
\includegraphics[width=0.433\textwidth]{img/l-shape_eig1_mesh.png}
\end{center}
\vspace{-5mm}
...
\includegraphics[width=0.433\textwidth]{img/l-shape_eig2_mesh.png}
\end{center}
\vspace{-5mm}
...
As can be seen in Figures~\ref{fig:l_sh_eig1} and \ref{fig:l_sh_eig2} we presented the first two eigenfunctions of the L-shaped domain with their corresponding adapted meshes.
We choose this example because it is a classical one, in the sense that it well known that for this problem the regularity of the eigenfunction could vary. Here we have the first eigenfunctions that exhibits  
a singularity in the gradient at the reentering corner. On the other hand, on the same domain with a reentering corner, the second eigenfunction is completely smooth. The differences in the regularity are reflected in the adapted meshes: for the first eigenfunction a great amount of $h$-refinement at the reentering corner, which is the correct way to tackle this kind of singularities, instead, for the second eigenfunction we have an adapted mesh mostly characterized by $p$-refinement, which is suitable for smooth functions.
